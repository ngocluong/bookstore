...
When(/^I search with the title of the first book$/) do
  step %{I fill in "q" with "#{@books.first.title}"}
  step %{I press "Go"}
end
...
When(/^I choose category of the first book$/) do
  page.select(@categories.first.name, from: 'category_id')
end

When(/^I search with the author name of the first book$/) do
  step %{I fill in "q" with "#{@books.first.author_name}"}
  step %{I press "Go"}
end

When(/^I search with the title is not available$/) do
  step %{I fill in "q" with "something else"}
  step %{I press "Go"}
end

When(/^I choose category not contain the first book$/) do
  page.select(@categories.last.name, from: 'category_id')
end
